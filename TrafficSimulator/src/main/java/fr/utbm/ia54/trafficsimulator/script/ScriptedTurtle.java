...
public abstract class ScriptedTurtle<C extends ScriptExecutionContext> extends Turtle {
	...
	public static final String ACTIVATE_SCRIPT_FUNCTION = "activateTurtle"; //$NON-NLS-1$
	
	private static final String BEHAVIOUR_SCRIPT_FUNCTION = "behaviourTurtle"; //$NON-NLS-1$
	
	public static final String END_SCRIPT_FUNCTION = "endTurtle"; //$NON-NLS-1$
	 ...
	 */
	protected ScriptedTurtle(C interpreter, ScriptLoader<C> startupScriptLoader) {
		super();
	 ...
	 * @param interpreter is the script interpreter to use.
	 */
	public ScriptedTurtle(C interpreter) {
	 ...
	 */
	public ScriptedTurtle(C interpreter, String scriptBasename) {
	 ...
	 */
	public ScriptedTurtle(C interpreter, File script) {
	 ...
	 * @param script is the filename of the script to load at startup.
	 */
	public ScriptedTurtle(C interpreter, URL script) {
	...
	private Status run(String name, boolean ignoreMissedMethod) {
		...
		try {
			...
			status = StatusFactory.error(ScriptedTurtle.this,
	...
	protected static class RepositoryScriptLoader<C extends ScriptExecutionContext> extends ScriptLoader<C> {
		 ...
		 */
		@Override
		protected void loadStartupScript(ScriptExecutionContext context, ScriptedTurtle<C> agent) throws IOException {
