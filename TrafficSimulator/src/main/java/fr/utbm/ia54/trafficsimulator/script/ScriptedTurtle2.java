...
import org.janusproject.scriptedagent.ScriptPath;
import org.janusproject.scriptedagent.ScriptRepository;
 ...
 * @since 0.5
 */
public abstract class ScriptedTurtle2<C extends ScriptExecutionContext> extends Turtle {
    ...
    private static final String BEHAVIOUR_SCRIPT_FUNCTION = "behaviourTurtle"; //$NON-NLS-1$
	...
	public static ScriptEngineManager getSharedScriptEngineManager() {
		synchronized(ScriptedTurtle2.class) {
	 ...
	 */
	protected ScriptedTurtle2(C interpreter, ScriptLoader<C> startupScriptLoader) {
		super();
	 ...
	 * @param interpreter is the script interpreter to use.
	 */
	public ScriptedTurtle2(C interpreter) {
	 ...
	 */
	public ScriptedTurtle2(C interpreter, String scriptBasename) {
	 ...
	 */
	public ScriptedTurtle2(C interpreter, File script) {
	 ...
	 * @param script is the filename of the script to load at startup.
	 */
	public ScriptedTurtle2(C interpreter, URL script) {
    ...
    protected void turtleBehavior() {
     // Run the living function from the script.
        if (this.automaticScriptExecution) {
            run(BEHAVIOUR_SCRIPT_FUNCTION, false);
        }
    }
	...
	private Status run(String name, boolean ignoreMissedMethod) {
		...
		try {
			...
			status = StatusFactory.error(ScriptedTurtle2.this,
					Locale.getString(ScriptedTurtle2.class,
	...
	protected static abstract class ScriptLoader<C extends ScriptExecutionContext> {
		 ...
		 * @throws IOException
		 */
		protected abstract void loadStartupScript(ScriptExecutionContext context, ScriptedTurtle2<C> agent) throws IOException;
		 ...
		 */
		Status load(ScriptExecutionContext context, ScriptedTurtle2<C> agent) {
			Status status = null;
			boolean isCatched = context.isCatchAllExceptions();
			context.setCatchAllExceptions(false);
			try {
	...
	protected static class RepositoryScriptLoader<C extends ScriptExecutionContext> extends ScriptLoader<C> {
		 ...
		 */
		@Override
		protected void loadStartupScript(ScriptExecutionContext context, ScriptedTurtle2<C> agent) throws IOException {
